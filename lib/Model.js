...
function parseKers(line) {
  if (typeof line !== "string") return null
  var f = line.split("\t")
  if (f.length < 4 || f[0] !== "kers") return null
  var rx = Number(f[2])
  var tx = Number(f[3])
...
// Kernel counters (kers) are absolute since boot (e.g., from /proc/net/dev),
// whereas app rows are relative to the collector's start. Therefore, the first
// sight of a kernel counter establishes a baseline rather than being a delta itself.
function kersDelta(prev, rx, tx) {
  var r = (isFinite(rx) && rx >= 0) ? Number(rx) : 0
  var t = (isFinite(tx) && tx >= 0) ? Number(tx) : 0
  if (!prev) return { dRx: 0, dTx: 0 }
  var dRx = r - (Number(prev.rx) || 0)
  var dTx = t - (Number(prev.tx) || 0)
  if (dRx < 0) dRx = r
  if (dTx < 0) dTx = t
  return { dRx: dRx, dTx: dTx }
}
...
function capApps(day, limit) {
  ...
  var out = { up: day.up, down: day.down, apps: apps }
  if (day.kUp !== undefined) out.kUp = day.kUp
  if (day.kDown !== undefined) out.kDown = day.kDown
  if (day.restarts !== undefined) out.restarts = day.restarts
  return out
}
...
function sanitizeStore(raw, maxDays, maxApps) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    var kUp2 = Number(d.kUp)
    var kDown2 = Number(d.kDown)
    var restarts2 = Number(d.restarts)
    ...
    if (isFinite(kUp2) && kUp2 >= 0) cleanDay.kUp = kUp2
    if (isFinite(kDown2) && kDown2 >= 0) cleanDay.kDown = kDown2
    if (isFinite(restarts2) && restarts2 >= 0) cleanDay.restarts = Math.floor(restarts2)
    
    out[keys[i]] = capApps(cleanDay, maxApps)
  }
  return out
}
...
function wireDown(day) {
  if (!day) return 0
  if (day.kDown !== undefined) return Number(day.kDown) || 0
  return Number(day.down) || 0
}

function wireUp(day) {
  if (!day) return 0
  if (day.kUp !== undefined) return Number(day.kUp) || 0
  return Number(day.up) || 0
}

function divergenceNote(day) {
  if (!day || day.kDown === undefined) return ""
  var kDown = Number(day.kDown) || 0
  var down = Number(day.down) || 0
  if (kDown >= 20 * 1024 * 1024 && down > kDown * 2) {
    var multiple = Math.round((down / kDown) * 10) / 10
    return "counts exceed wire x" + multiple + " - run net-usage validate"
  }
  return ""
}
